...
export default function ProjectCard({ project, requestDelete }) {
    const { id, title, student, category, desc, image, driveLink } = project;
    ...
    return (
        <div className="group relative bg-[#18181b]/60 backdrop-blur-md border border-white/5 rounded-2xl md:rounded-3xl overflow-hidden hover:-translate-y-2 transition-all duration-500 hover:shadow-2xl hover:shadow-purple-500/10 hover:border-purple-500/20 flex flex-col">
            ...
            <div className="p-5 md:p-6 flex flex-col flex-grow">
                ...
                <p className="text-xs md:text-sm text-gray-400 mb-3 flex items-center gap-2"><span className="w-1 h-1 rounded-full bg-gray-500"></span> {student}</p>
                <p className="text-gray-500 text-xs md:text-sm mb-5 line-clamp-2">{desc}</p>
                <a href={driveLink} target="_blank" rel="noreferrer" className="mt-auto w-full flex items-center justify-center gap-2 py-2.5 md:py-3 rounded-xl bg-white/5 border border-white/10 text-white font-medium text-xs md:text-sm hover:bg-white hover:text-black transition-all duration-300">
